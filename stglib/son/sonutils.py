...
def parse_pingHeader(fheader, timeprefix):
    ...
    temp = [128, 40]
    pingheader["SwitchCommandBytes"] = temp[fheader[3] % 2]
    temp = [256, 32]
    pingheader["ReturnHeaderBytes"] = temp[fheader[3] % 2]

    # Get file size
    pingheader["TotalBytes"] = struct.unpack("<I", fheader[4:8])[0]

    # Extract additional information
    pingheader["Orientation"] = bin(fheader[319])[2:].zfill(8)[7]  # 1 is up
    temp = ["Sector", "Polar", "Sidescan"]
...
def parse_switchCommand(scommand):
    ...
    SwitchCommand["SectorSize"] = 3 * scommand[12]
...
def parse_returnHeader(sheader, switches):
    ...
    returnHeader["Angle"] = 0.3 * (returnHeader["SonarPosition"] - 600)
